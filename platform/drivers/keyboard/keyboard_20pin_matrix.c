...
keyboard_t kbd_getKeys()
{
    ...
    delayUs(1);
    if(gpio_readPin(KB_ROW0) == 0) keys |= KEY_F1;
    if(gpio_readPin(KB_ROW1) == 0) keys |= KEY_1;
    if(gpio_readPin(KB_ROW2) == 0) keys |= KEY_4;
    if(gpio_readPin(KB_ROW3) == 0) keys |= KEY_7;
    ...
    if(gpio_readPin(KB_ROW4) == 0) keys |= KEY_0;
    gpio_setPin(KB_COL1);

    gpio_clearPin(KB_COL2);
    delayUs(1);
    if(gpio_readPin(KB_ROW0) == 0) keys |= KEY_HASH;
    if(gpio_readPin(KB_ROW1) == 0) keys |= KEY_3;
    if(gpio_readPin(KB_ROW2) == 0) keys |= KEY_6;
    if(gpio_readPin(KB_ROW3) == 0) keys |= KEY_9;
    ...
    if(gpio_readPin(KB_ROW1) == 0) keys |= KEY_UP;
    if(gpio_readPin(KB_ROW2) == 0) keys |= KEY_DOWN;
    if(gpio_readPin(KB_ROW3) == 0) keys |= KEY_ESC;
    if(gpio_readPin(KB_ROW4) == 0) keys |= KEY_ENTER;
    gpio_setPin(KB_COL3);
