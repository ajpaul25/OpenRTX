...
void gps_uartPrint(const char *fmt, ...)
{
    char buf[NMEA_MSG_SIZE] = { 0 };
    va_list args;
    va_start(args, fmt);
    vsnprintk(buf, NMEA_MSG_SIZE, fmt, args);
    int msg_len = strnlen(buf, NMEA_MSG_SIZE);
    for (uint16_t i = 0; i < msg_len; i++) {
        uart_poll_out(gps_dev, buf[i]);
    }
    va_end(args);
}

void gps_uartScan(char *buf)
{
