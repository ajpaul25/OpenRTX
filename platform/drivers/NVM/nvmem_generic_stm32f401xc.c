...
typedef struct
{
    uint16_t     crc; //2 types
    settings_t   settings; //21 bytes
}
__attribute__((packed)) dataBlock_t; //23 bytes

typedef struct
{
    uint32_t    magic; //4 bytes
    uint32_t    flags[32]; //128 bytes
    dataBlock_t data[1024]; //23552 bytes
}
__attribute__((packed)) memory_t; //23684 bytes ~ 24k

static const uint32_t MEM_MAGIC   = 0x584E504F;    // "OPNX"
static const uint32_t baseAddress = 0x08034000;
...
static int findActiveBlock()
{
    ...
    // Check data validity
    const size_t crcLen = sizeof(settings_t);
